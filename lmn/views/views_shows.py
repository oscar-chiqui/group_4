...
def shows_with_most_notes(request):
    ...
    # Get the top ten shows with the most notes by getting the count and ordering by number of notes. Exclude shows with 0 notes.
    top_10_shows = Show.objects.annotate(num_notes=Count('note')).exclude(num_notes=0).order_by('-num_notes')[:10]  # Adapted from Django docs https://docs.djangoproject.com/en/4.2/topics/db/aggregation/
